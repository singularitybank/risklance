...
class DataManager {
    ...
    loadDummyData() {
        ...
        // 保険カバー率データ
        this.data.insuranceCoverage = [
            { category: '火災', percent: 85 },
            { category: '賠償', percent: 70 },
            { category: '傷害・医療', percent: 90 },
            { category: '自動車', percent: 100 }
        ];

        // おすすめ保険見直しデータ
        this.data.insuranceRecommendations = [
            {
                id: 1,
                title: 'サイバーセキュリティ保険の追加',
                description: 'IT資産の増加に伴い、サイバーリスクが高まっています。年間保険料: ¥120,000〜',
                icon: 'fas fa-lightbulb'
            },
            {
                id: 2,
                title: '火災保険の補償額見直し',
                description: '事業規模拡大により、現在の補償額では不足の可能性があります。',
                icon: 'fas fa-lightbulb'
            }
        ];
        ...
        this.data.actionItems = [
            {
                ...
                level: 'high',
                title: '火災保険の補償内容見直し',
                description: '現在の事業規模に対して補償額が不足している可能性があります',
                action: '対応する',
            ...
            },
            {
                id: 2,
                level: 'medium',
                title: 'サイバーセキュリティ保険の検討',
                description: 'IT資産の増加に伴い、サイバーリスクが高まっています',
                action: '詳細を見る',
                icon: 'fas fa-exclamation-triangle'
            }
        ];
        ...
        this.data.internalRiskData = {
            inventory: {
                trends: [
                    { month: '4月', value: '65M', height: '65%' },
                    { month: '5月', value: '70M', height: '70%' },
                    { month: '6月', value: '74M', height: '74%' },
                    { month: '7月', value: '80M', height: '80%' },
                    { month: '8月', value: '81M', height: '81%' },
                    { month: '9月', value: '82M', height: '82%' }
                ],
                riskLevel: 'high',
                riskText: '高リスク',
                coverage: { status: 'partial', text: '火災保険: 61%カバー（5,000万円/8,200万円）' }
            },
            employees: {
                safe: 40,
                caution: 35,
        ...
        this.data.externalRiskData = {
            ...
            disasterRiskText: '高リスク',
            disasterCoverage: { status: 'partial', text: '災害保険: 部分カバー' },
            creditRisk: [
                {
                    company: 'ABC商事',
                    score: 85,
            ...
            creditRiskText: '高リスク',
            creditCoverage: { status: 'partial', text: '取引信用保険: 60%カバー' },
            marketRisk: {
                gauge: 45, // 度数（0-180）
                factors: [
    ...
    // 保険ポートフォリオデータ取得
    getInsurancePortfolioData() {
        return {
            policies: this.data.insuranceData,
            coverage: this.data.insuranceCoverage,
            recommendations: this.data.insuranceRecommendations
        };
    }
    ...
    getRiskAnalysisData() {
        return {
            ...
            emergencyAlerts: this.data.emergencyAlerts,
            internalRisk: this.data.internalRiskData,
            externalRisk: this.data.externalRiskData,
            summary: this.data.riskSummary
        };
    }
    ...
    getRiskBrandingData() {
        return {
            league: this.data.leagueData,
            guild: this.data.guildData
        };
    }
}
